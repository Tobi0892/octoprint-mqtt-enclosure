...
class CCS811:
    ...
    def __init__(self, twi=1, addr=CCS811_ADDRESS):
        ...
        self.hwid = self.checkHWID()

        if self.hwid != hex(129):
            print('Incorrect hardware ID ', self.hwid, ', should be 0x81')

        self.configureSensor(0b100000)

    def read(self, temperature, humidity):
        self.setCompensation(temperature, humidity)
        status = self.readStatus()

        if self.checkError(status) or not self.checkDataReady(status):
            ...
            result = self.readAlg()
            return {
                "eCO2": self.formatText(result['eCO2']),  # ppm
                "TVOC": self.formatText(result['TVOC']),  # ppb
            }
    ...
    # original functions: https://gist.github.com/xxlukas42/60ae08f75e68a0cfcdb7c9dd60145d34
    def checkHWID(self):
